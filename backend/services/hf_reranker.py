...
class HuggingFaceReranker:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        logger.info("Initialized HuggingFaceReranker with model %s", self.model_name)

    async def rerank(self, query: str, chunks: list[RetrievedChunk], top_k: int) -> list[RetrievedChunk]:
        """Score and reorder ``chunks`` by relevance to ``query`` via Hugging Face Serverless."""
        if not chunks:
            return chunks[:top_k]

        if not self.settings.hf_token:
            logger.warning("HF_TOKEN is missing. Skipping HF reranking and returning top candidates.")
            return chunks[:top_k]
