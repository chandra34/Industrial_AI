...
class RCAEvent(Base):
    """Stores structured Root Cause Analysis reports generated autonomously or on-demand."""

    __tablename__ = "rca_events"

    id = Column(String, primary_key=True, doc="Unique RCA Event ID (e.g. 'rca_a1b2c3d4')")
    asset_name = Column(String, nullable=False, index=True, doc="Human-readable machine name (e.g. 'Boiler Feed Pump 02')")
    node_id = Column(String, nullable=False, index=True, doc="OPC UA Node ID of triggering sensor/machine")
    sap_equipment_id = Column(String, nullable=True, index=True, doc="Associated SAP Equipment ID if mapped")
    alarm_type = Column(String, nullable=False, doc="Alarm or Anomaly description (e.g. 'High Vibration Trip (4.8 mm/s)')")
    severity = Column(String, nullable=False, default="CRITICAL", doc="'WARNING', 'CRITICAL', or 'EMERGENCY'")
    root_cause = Column(Text, nullable=False, doc="Synthesized primary root cause summary")
    confidence_score = Column(Integer, nullable=False, default=85, doc="AI confidence percentage (0 to 100)")
    telemetry_evidence = Column(Text, nullable=True, doc="Telemetry & anomaly findings summary")
    sap_evidence = Column(Text, nullable=True, doc="SAP maintenance notifications & work orders summary")
    sop_evidence = Column(Text, nullable=True, doc="OEM Manual section & SOP procedure summary")
    recommended_actions_json = Column(Text, nullable=True, doc="JSON-serialized list of recommended corrective steps")
    raw_llm_response = Column(Text, nullable=True, doc="Full markdown answer generated by the agent")
    status = Column(String, nullable=False, default="open", doc="'open', 'acknowledged', or 'resolved'")
